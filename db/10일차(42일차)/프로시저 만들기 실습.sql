...
begin
    ...
    if _seat1 is not null then
		set _count = _count +1;
        set _total_price = _total_price + (select se_price from screen_schedule join cinema on ss_ci_num = ci_num
        join seat on se_ci_num = ci_num where ss_num = _ss_num and se_name = _seat1);
	end if;
    if _seat2 is not null then
		set _count = _count +1;
        set _total_price = _total_price + (select se_price from screen_schedule join cinema on ss_ci_num = ci_num
        join seat on se_ci_num = ci_num where ss_num = _ss_num and se_name = _seat2);
	end if;
    if _seat3 is not null then 
		set _count = _count +1;
        set _total_price = _total_price + (select se_price from screen_schedule join cinema on ss_ci_num = ci_num
        join seat on se_ci_num = ci_num where ss_num = _ss_num and se_name = _seat3);
	end if;
 ...
 if _seat1 is not null then
        ...
        insert into ticketing_seat(ts_ti_num, ts_se_num) value(_ti_num, _se_num);
	end if;

	insert ticketing_seat(ts_se_num, ts_ti_num) value ((select se_num from screen_schedule join cinema on ss_ci_num = ci_num
        join seat on se_ci_num = ci_num where ss_num = _ss_num and se_name = _seat1), (select max(ti_num) from ticketing));
    insert ticketing_seat(ts_se_num, ts_ti_num) value ((select se_num from screen_schedule join cinema on ss_ci_num = ci_num
        join seat on se_ci_num = ci_num where ss_num = _ss_num and se_name = _seat1), (select max(ti_num) from ticketing)); 
	insert ticketing_seat(ts_se_num, ts_ti_num) value ((select se_num from screen_schedule join cinema on ss_ci_num = ci_num
        join seat on se_ci_num = ci_num where ss_num = _ss_num and se_name = _seat1),  );
