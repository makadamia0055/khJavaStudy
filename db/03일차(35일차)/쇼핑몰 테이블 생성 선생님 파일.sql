...
create table if not exists product_category(
    ...
    pc_name varchar(10) not null -- 제품 카테고리 명
...
create table if not exists board(
    ...
    bo_reg_date datetime not null default now(), -- now() 현재시간 
...
create table if not exists buy(
    ...
    bu_me_id varchar(13) not null, -- 구매 아이디
...
create table if not exists inquiry(
    ...
    in_reply longtext, -- 답변
    in_pw char(4), -- 문의 비번
    in_me_id varchar(15) not null, -- 문의 아이디
    in_pr_num int not null, -- 문의 제품
    constraint in_bu_me_id foreign key(in_me_id) references member(me_id),
    constraint in_bu_pr_num foreign key(in_pr_num) references product(pr_num)
);
